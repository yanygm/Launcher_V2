                ...
                while (!pinRestored && checkCount < 30) // 最多检测30秒
                {
                    if (CheckTcpConnection(processId, serverIP, serverPort))
                    {
                        ...
                        if (RestorePinFile())
                        {
                            ...
                            // 修改内存
                            // 星标赛道数量50改为120
                            ModifyMemory(processId, new byte[] { 0x80, 0xC2, 0x01, 0x8B, 0x45, 0xF8, 0x88, 0x90, 0x94, 0x0A, 0x00, 0x00 },
                            new byte[] { 0x80, 0xC2, 0x00, 0x8B, 0x45, 0xF8, 0x88, 0x90, 0x94, 0x0A, 0x00, 0x00 });
